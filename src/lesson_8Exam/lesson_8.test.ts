import {getBanknoteLis, getSum, getTriangleType, isEvenIndexSumGreater, getSquarePositiveInteger, sum, sumFirstNumber} from "./lesson_8";
...
test("get Square Only Of Positive Integers", () => {
    ...
    const result = getSquarePositiveInteger(array)
...
test("sum of first N number", () => {
    expect(sumFirstNumber(0)).toBe(0)
    expect(sumFirstNumber(4)).toBe(10)
    expect(sumFirstNumber(10)).toBe(55)
})
test("get banknote list", ()=> {
    // надо бы проверять длинну резалтов и их сумму
    const result2500 = getBanknoteLis(2500)
    const result23 = getBanknoteLis(23)
